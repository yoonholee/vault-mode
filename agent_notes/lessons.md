...
# Merged from vscode-markdown-css (2026-07-04)

## Consumer-side knowledge (VS Code preview + md-print)

Hard-won facts about the two renderers. The CSS is simple; the delivery is where the traps are.

## VS Code Markdown preview

The preview is VS Code's **built-in** Markdown preview (opened via the `preview-to-side` extension → `markdown.showPreviewToSide`), a Chromium webview. NOT Obsidian, so `.obsidian/` config changes do nothing.
...
2. Callout headers `> [!type] Title` → `> **Title**` (bold title in the blockquote). pandoc and VS Code's built-in preview don't know callout blocks, so without this the literal `[!type]` shows. Print is now cleaner than the actual preview here (fixing the preview needs this extension's markdown-it plugin).
