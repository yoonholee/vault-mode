...
import { VsClient, type Spawner } from "../../src/vsClient";

// Stub spawner: returns a fake child-process-like object with predetermined stdout/stderr/exit.
function fakeSpawn(opts: { stdout?: string; stderr?: string; exitCode?: number; delayMs?: number }): Spawner {
  return (_cmd: string, _args: string[]) => {
    ...
    const child = {
      stdout: { on: (ev: string, cb: (chunk: Buffer) => void) => { listeners.set("stdout." + ev, [...(listeners.get("stdout." + ev) ?? []), cb as unknown as (...a: unknown[]) => void]); } },
      stderr: { on: (ev: string, cb: (chunk: Buffer) => void) => { listeners.set("stderr." + ev, [...(listeners.get("stderr." + ev) ?? []), cb as unknown as (...a: unknown[]) => void]); } },
      on: (ev: string, cb: (...a: unknown[]) => void) => { listeners.set(ev, [...(listeners.get(ev) ?? []), cb]); },
      kill: () => fire("exit", 137),
    };
    setTimeout(() => {
      if (opts.stdout) (listeners.get("stdout.data") ?? []).forEach((cb) => cb(Buffer.from(opts.stdout!)));
      if (opts.stderr) (listeners.get("stderr.data") ?? []).forEach((cb) => cb(Buffer.from(opts.stderr!)));
